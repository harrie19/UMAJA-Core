...
**Status:** {report['checks']['backend_url']['status']}  
**Message:** {report['checks']['backend_url']['message']}

---

## Bug Scan

**Status:** {report['checks']['bug_scan']['status']}  
**Total Findings:** {report['checks']['bug_scan']['total_findings']}

### By Severity:
- Critical: {report['checks']['bug_scan']['by_severity']['CRITICAL']}
- High: {report['checks']['bug_scan']['by_severity']['HIGH']}
- Medium: {report['checks']['bug_scan']['by_severity']['MEDIUM']}
- Low: {report['checks']['bug_scan']['by_severity']['LOW']}

---

## Test Status

**Status:** {report['checks']['test_status']['status']}  
**Pytest Available:** {report['checks']['test_status']['pytest_available']}  
**Test Files:** {report['checks']['test_status']['test_files_count']}

---

## Deployment Files

**Status:** {report['checks']['deployment_files']['status']}  
**All Present:** {report['checks']['deployment_files']['all_present']}

Missing Files: {', '.join(report['checks']['deployment_files']['missing_files']) or 'None'}
...
def main():
    ...
    print("=" * 60)
    print("Philosophy: Reality Glasses (Verify, Don't Assume)")
    print("=" * 60)
    print()
    ...
    print(f"Results saved to: data/reality_checks/")
    print()
